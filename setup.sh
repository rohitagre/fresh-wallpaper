...
if [[ $response =~ ^[Yy]$ ]]; then
    ...
    if [[ -z "$cat" ]]; then
        ...
        echo "30 11 * * * curl -L --compressed "https://source.unsplash.com/1920x1080/?$cat" -o ~/Pictures/unsplash-wallpapers/\`date +"\\%d\\%m\\%Y\\%H"\`.jpg  >/dev/null 2>&1 #fresh-wallpaper" >> mycron.txt
    fi

    echo "35 11 * * * osascript -e 'tell application \"System Events\" to tell every desktop to set picture to \"~/Pictures/unsplash-wallpapers/\`date +'\\%d\\%m\\%Y\\%H'\`.jpg\"' #fresh-wallpaper" >> mycron.txt
